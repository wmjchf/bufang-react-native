...
export const getCategoryListData = (data) => {
  return async (dispatch) => {
    const resultLocal = await dataStore.fetchLocalData({url: '/playlist/hot'});
    let tags = resultLocal && handleTagsData(resultLocal.tags);
    tags && dispatch(handleSuccess(tags, false));
    const resultNet = await dataStore.fetchNetGetData({url: '/playlist/hot'});
    tags = handleTagsData(resultNet.tags);
    dispatch(handleSuccess(tags, true));
  };
};
